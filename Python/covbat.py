...
def covbat(data, batch, model=None, numerical_covariates=None, pct_var=0.80, n_pc=0):
    ...
    bmu = np.mean(bayesdata, axis=0)
    pca = PCA()
    pca.fit(bayesdata)
    full_scores = pca.fit_transform(bayesdata)
    ...
    pca.fit(bayesdata)
    scores = pca.fit_transform(bayesdata)
    scores_com <- combat(scores.transpose(), bat, eb=False)

    full_scores[:,range(0,npc)] = scores_com.transpose()
    x_covbat = np.dot(full_scores, pca.components_) + bmu
